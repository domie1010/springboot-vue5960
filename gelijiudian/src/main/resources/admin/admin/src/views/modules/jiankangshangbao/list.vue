        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '现在状态' : ''">
                        <el-select v-model="searchForm.jiankangshangbaoTypes" placeholder="请选择现在状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in jiankangshangbaoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('jiankangshangbao','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('jiankangshangbao','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('jiankangshangbao','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jiankangshangbao','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gelijiudian/upload/jiankangshangbaoMuBan.xls"
                        >批量导入健康上报数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jiankangshangbao','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jiankangshangbaoUploadSuccess"
                                :on-error="jiankangshangbaoUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jiankangshangbao','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jiankangshangbao.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jiankangshangbaoUuidNumber"
                                   header-align="center"
                                   label="健康上报编号">
                        <template slot-scope="scope">
                            {{scope.row.jiankangshangbaoUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jiankangshangbaoTypes"
                                      header-align="center"
                                      label="现在状态">
                        <template slot-scope="scope">
                            {{scope.row.jiankangshangbaoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jiankangshangbaoTiwenPhoto"
                                     header-align="center"
                                     width="200"
                                     label="核酸结果照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.jiankangshangbaoTiwenPhoto">
                                <img :src="$base.url+scope.row.jiankangshangbaoTiwenPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jiankangshangbaoTiwen"
                                   header-align="center"
                                   label="体温">
                        <template slot-scope="scope">
                            {{scope.row.jiankangshangbaoTiwen}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jiankangshangbao','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jiankangshangbao','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jiankangshangbao','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              jiankangshangbaoTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                jiankangshangbaoUuidNumber : null,
                jiankangshangbaoTypes : null,
                jiankangshangbaoTiwenPhoto : null,
                jiankangshangbaoTiwen : null,
                jiankangshangbaoContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '健康上报编号': "jiankangshangbaoUuidNumber",
                     '现在状态': "jiankangshangbaoValue",
                     '核酸结果照片': "jiankangshangbaoTiwenPhoto",
                     '体温': "jiankangshangbaoTiwen",
                     '上报时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jiankangshangbao',//当前表表名,
                        sumColum : 'jiankangshangbao_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jiankangshangbao_name',//分组字符串字段
                        // types : 'jiankangshangbao_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jiankangshangbao_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.jiankangshangbaoTypes!= '' && this.searchForm.jiankangshangbaoTypes!= undefined) {
                    params['jiankangshangbaoTypes'] = this.searchForm.jiankangshangbaoTypes
                }
                                        
                params['jiankangshangbaoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=jiankangshangbao_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.jiankangshangbaoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            jiankangshangbaoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jiankangshangbao/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            jiankangshangbaoUploadError(data){
